...
RED.history = (function() {
    ...
    return {
        ...
        markAllDirty: function() {
            // In live mode, markAllDirty is a no-op (no deploy boundary)
        ...
        push: function(ev) {
            ...
            // Live sync: publish change to server
            if (RED.liveSync && RED.liveSync.isActive()) {
                RED.liveSync.publishChange(ev);
            }
        },
        pop: function() {
            ...
            // Live sync: publish the inverse (undo) event
            if (RED.liveSync && RED.liveSync.isActive() && rev) {
                RED.liveSync.publishChange(rev);
            }
        },
        peek: function() {
        ...
        redo: function() {
            ...
            if (ev) {
                ...
                // Live sync: publish the re-applied event
                if (RED.liveSync && RED.liveSync.isActive() && uev) {
                    RED.liveSync.publishChange(uev);
                }
            }
            RED.menu.setDisabled("menu-item-edit-undo", undoHistory.length === 0);
            RED.menu.setDisabled("menu-item-edit-redo", redoHistory.length === 0);
