RED.multiplayer = (function () {
    ...
    // Baton (merge-master) state
    let _batonHolder = null           // { session, user } or null
    let _batonPassNotification = null // active pass-request notification
    ...
    function handleBatonPassRequest(requester) {
        if (_batonPassNotification) {
            _batonPassNotification.close()
        }
        var username = (requester && requester.user && requester.user.username) || 'Someone'
        var message = RED._('livesync.baton.passRequest', { user: username }) ||
            (username + ' is requesting the baton')

        _batonPassNotification = RED.notify(message, {
            type: 'warning',
            fixed: true,
            timeout: 10000,
            buttons: [
                {
                    text: RED._('livesync.baton.pass') || 'Pass',
                    class: 'primary',
                    click: function() {
                        RED.comms.send('multiplayer/baton-request', {
                            action: 'pass',
                            target: requester.session
                        })
                        _batonPassNotification.close()
                        _batonPassNotification = null
                    }
                },
                {
                    text: RED._('livesync.baton.decline') || 'Decline',
                    click: function() {
                        _batonPassNotification.close()
                        _batonPassNotification = null
                    }
    ...
    return {
        getSessionId: function() { return activeSessionId },
        getBatonHolder: function() { return _batonHolder },
        isLocalBatonHolder: function() {
            return _batonHolder && _batonHolder.session === activeSessionId
        },
        requestBaton: function() {
            RED.comms.send('multiplayer/baton-request', { action: 'request' })
        },
        releaseBaton: function() {
            RED.comms.send('multiplayer/baton-request', { action: 'release' })
        },
        init: function () {
            ...
            RED.comms.subscribe('multiplayer/#', (topic, msg) => {
                ...
                if (topic === 'multiplayer/init') {
                    ...
                    // Initialise baton state
                    _batonHolder = msg.baton || null
                    RED.events.emit('baton:state-changed', { holder: _batonHolder })
                ...
                } else if (topic === 'multiplayer/lock-granted') {
                    log('lock granted', msg.nodeId)
                } else if (topic === 'multiplayer/lock-denied') {
                    var holderName = msg.holder && msg.holder.user ? msg.holder.user.username : 'another user'
                    RED.notify(RED._("livesync.lock.denied", { user: holderName }) || ("Node is being edited by " + holderName), "warning")
                    // Close the edit tray if it's still opening
                    RED.tray.close()
                } else if (topic === 'multiplayer/lock-changed') {
                    // Update lock visual on canvas
                    var node = RED.nodes.node(msg.nodeId)
                    if (node) {
                        if (msg.locked) {
                            node._lock = { session: msg.session, user: msg.user }
                        } else {
                            delete node._lock
                        }
                        node._multiplayer_refresh = true
                ...
                } else if (topic === 'multiplayer/baton-state') {
                    _batonHolder = msg.holder || null
                    RED.events.emit('baton:state-changed', { holder: _batonHolder })
                } else if (topic === 'multiplayer/baton-granted') {
                    _batonHolder = { session: msg.session, user: msg.user }
                    RED.events.emit('baton:state-changed', { holder: _batonHolder })
                    // Close any pending pass-request notification
                    if (_batonPassNotification) {
                        _batonPassNotification.close()
                        _batonPassNotification = null
                    }
                } else if (topic === 'multiplayer/baton-pass-request') {
                    handleBatonPassRequest(msg.requester)
                }
            })
