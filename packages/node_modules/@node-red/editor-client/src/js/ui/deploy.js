...
RED.deploy = (function() {
    ...
    function init(options) {
        ...
        // Live editing mode: replace deploy button with live status indicator
        if (RED.settings.theme("liveEditing.enabled", false)) {
            initLiveStatusIndicator(type);
        }

        window.addEventListener('beforeunload', function (event) {
            if (RED.nodes.dirty() && !RED.settings.theme("liveEditing.enabled", false)) {
                event.preventDefault();
        ...
        RED.events.on('workspace:dirty',function(state) {
            ...
            // In live mode, don't toggle deploy button
            if (RED.settings.theme("liveEditing.enabled", false)) {
                return;
            }
            if (state.dirty) {
        ...
        RED.comms.subscribe("notification/runtime-deploy",function(topic,msg) {
            ...
            // When live sync is active, handle revision updates
            if (RED.liveSync && RED.liveSync.isActive()) {
                if (msg.patch) {
                    // Change came from patchFlows (liveSync) - the livesync/applied
                    // handler already takes care of applying remote changes and
                    // updating the revision. Just update rev here to prevent stale
                    // checks and skip the diff/merge entirely.
                    RED.nodes.version(msg.revision);
                    return;
                }
                // Non-patch deploy (e.g. another client did a full deploy) -
                // auto-merge if no conflicts, otherwise show review dialog
                RED.diff.getRemoteDiff(function(diff) {
                    var conflicts = Object.keys(diff.conflicts);
                    if (conflicts.length === 0) {
                        RED.diff.mergeDiff(diff);
                        RED.notify(RED._('deploy.confirm.autoMerged'), {
                            type: 'compact',
                            timeout: 3000
                        });
                    } else {
    ...
    function initLiveStatusIndicator(type) {
        // Replace deploy button content with live status indicator
        var deployBtn = $("#red-ui-header-button-deploy");
        deployBtn.addClass("disabled");
        deployBtn.find(".red-ui-deploy-button-content").html(
            '<span class="red-ui-live-status">' +
                '<span class="red-ui-live-status-dot live"></span>' +
                '<span class="red-ui-live-status-label">' + RED._("livesync.status.live") + '</span>' +
            '</span>'
        );
        deployBtn.off("click");

        // Add baton indicator button next to the deploy button group
        var batonBtn = $('<a id="red-ui-baton-btn" class="red-ui-baton-btn" href="#"></a>');
        batonBtn.text(RED._("livesync.baton.take") || "Take Baton");
        batonBtn.insertAfter($(".red-ui-deploy-button-group"));
        batonBtn.on("click", function(event) {
            event.preventDefault();
            if (!RED.multiplayer) return;
            if (RED.multiplayer.isLocalBatonHolder()) {
                RED.multiplayer.releaseBaton();
            } else {
                RED.multiplayer.requestBaton();
            }
        });

        // Replace dropdown menu items for live mode
        if (type === "default") {
            var optionsBtn = $("#red-ui-header-button-deploy-options");
            var liveMenuItems = [];
            liveMenuItems.push({
                id: "deploymenu-item-snapshot",
                label: RED._("livesync.menu.saveSnapshot") || "Save Snapshot",
                onselect: function() { saveSnapshot(); }
            });
            liveMenuItems.push(null);
            if (RED.settings.runtimeState && RED.settings.runtimeState.ui === true) {
                liveMenuItems.push({id:"deploymenu-item-runtime-start", icon:"red/images/start.svg",label:RED._("deploy.startFlows"),sublabel:RED._("deploy.startFlowsDesc"),onselect:"core:start-flows", visible:false});
                liveMenuItems.push({id:"deploymenu-item-runtime-stop", icon:"red/images/stop.svg",label:RED._("deploy.stopFlows"),sublabel:RED._("deploy.stopFlowsDesc"),onselect:"core:stop-flows", visible:false});
            }
            liveMenuItems.push({id:"deploymenu-item-reload", icon:"red/images/deploy-reload.svg",label:RED._("deploy.restartFlows"),sublabel:RED._("deploy.restartFlowsDesc"),onselect:"core:restart-flows"});
            RED.menu.init({id:"red-ui-header-button-deploy-options", options: liveMenuItems });
        }

        // Listen for live sync status changes
        RED.events.on("livesync:status-changed", function(data) {
            var dot = deployBtn.find(".red-ui-live-status-dot");
            var label = deployBtn.find(".red-ui-live-status-label");
            dot.removeClass("live applying error disconnected watching pending");
            dot.addClass(data.status);
            var statusKey = "livesync.status." + data.status;
            label.text(RED._(statusKey) || data.status);
        });

        // Listen for baton state changes to update baton button
        RED.events.on("baton:state-changed", function(data) {
            var holder = data.holder;
            batonBtn.removeClass("holder spectator");
            if (!holder) {
                // No holder — show "Take Baton"
                batonBtn.text(RED._("livesync.baton.take") || "Take Baton");
            } else if (RED.multiplayer && RED.multiplayer.isLocalBatonHolder()) {
                // We hold the baton — show "Editing" (green)
                batonBtn.addClass("holder");
                batonBtn.text(RED._("livesync.baton.holding") || "Editing");
            } else {
                // Someone else holds the baton — show their name
                batonBtn.addClass("spectator");
                var username = (holder.user && holder.user.username) || "Someone";
                batonBtn.text(username);
            }
        });
    }

    function saveSnapshot() {
        // Save a full snapshot (equivalent to a deploy)
        var nns = RED.nodes.createCompleteNodeSet();
        var data = {flows: nns, rev: RED.nodes.version()};
        deployInflight = true;
        $.ajax({
            url: "flows",
            type: "POST",
            ...
            RED.notify(RED._("livesync.menu.saveSnapshot") + ": OK", "success");
        }).fail(function(xhr) {
            RED.notify(RED._("deploy.deployFailed", {message: xhr.responseText || "Error"}), "error");
        }).always(function() {
            deployInflight = false;
        });
    }
