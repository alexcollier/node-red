...
class Subflow extends Flow {
    ...
    constructor(parent,globalFlow,subflowDef,subflowInstance) {
        ...
        subflowInternalFlowConfig.subflows = subflowDef.subflows || {};
...
function createNodeInSubflow(subflowInstanceId, def) {
    let node = Object.assign({}, def);
    node._alias = def.id;
    node.id = `${subflowInstanceId}-${def.id}`;
    node.z = subflowInstanceId;
    // Deep clone only wires — mutated in-place by remapSubflowNodes
    if (node.wires) {
        node.wires = JSON.parse(JSON.stringify(node.wires));
    }
    // Clone scope array — reassigned by remapSubflowNodes for catch/complete/status
    if (Array.isArray(node.scope)) {
        node.scope = node.scope.slice();
    }
    return node;
}
