...
var _flowSaveDebounceMs = 2000; // default, overridden by settings.flowSaveDebounceMs

function scheduleDebouncedSave(user) {
    _savePending = true;
    if (_saveTimer) {
...
function _executeSave(user) {
    if (!_savePending) { return Promise.resolve(); }
    _savePending = false;
    var config = activeConfig.flows;
    return credentials.export().then(function(creds) {
        var saveConfig = {
            flows: config,
            credentialsDirty: credentials.dirty(),
            credentials: creds
        };
        return storage.saveFlows(saveConfig, user);
    }).then(function(flowRevision) {
        activeConfig.rev = flowRevision;
        log.debug("live-edit debounced save, revision: " + flowRevision);
        return flowRevision;
    }).catch(function(err) {
        log.warn("live-edit debounced save failed: " + err.toString());
    });
}

function flushSave(user) {
    if (_saveTimer) {
        clearTimeout(_saveTimer);
        _saveTimer = null;
    }
    return _executeSave(user);
}

// --- Change classification ---

/**
 * Classify a single change operation into a tier.
 * Tiers:
 *   0 - visual only (x, y, label, icon)
 *   1 - wiring change
 *   2 - node property edit
 *   3 - add node
 *   4 - remove node
 *   5 - config node edit
 *   6 - flow tab add/remove
 *   7 - subflow template edit
 */
function classifyChange(change) {
    if (change.op === 'add') {
        var node = change.node;
        if (node.type === 'tab') return 6;
        if (node.type === 'subflow') return 7;
        return 3;
    }
    if (change.op === 'remove') {
        var existing = activeFlowConfig.allNodes[change.nodeId];
        if (!existing) return -1;
        if (existing.type === 'tab') return 6;
        if (existing.type === 'subflow') return 7;
        return 4;
    }
    if (change.op === 'rewire') {
        return 1;
    ...
    // op === 'update'
    var nodeId = change.nodeId || (change.node && change.node.id);
    var existing = activeFlowConfig.allNodes[nodeId];
    if (!existing) return -1;
    if (existing.type === 'tab') return 6;
    if (existing.type === 'subflow') return 7;
    // Check if it's a config node (no x/y)
    if (!existing.hasOwnProperty('x') && !existing.hasOwnProperty('y')) {
        return 5;
    }
    // Check if only visual properties changed
    if (change.node) {
        var props = Object.keys(change.node).filter(function(k) {
            return k !== 'id' && k !== 'x' && k !== 'y' && k !== 'label' &&
                   k !== 'icon' && k !== 'w' && k !== 'h' && k !== 'wires';
        });
        if (props.length === 0) {
            // Check if wires also changed
            if (change.node.hasOwnProperty('wires')) return 1;
            return 0;
        }
    }
    return 2;
...
/**
 * Apply config-level changes to activeFlowConfig and activeConfig.
 * Returns an array of {nodeId, tier, oldConfig, newConfig} for runtime execution.
 */
function applyConfigChanges(changes) {
    var results = [];
    for (var i = 0; i < changes.length; i++) {
        var change = changes[i];
        var tier = classifyChange(change);
        var entry = { change: change, tier: tier, nodeId: null, oldConfig: null, newConfig: null };

        if (change.op === 'add') {
            var node = jsonClone(change.node);
            entry.nodeId = node.id;
            entry.newConfig = node;
            // Add to activeConfig.flows array
            activeConfig.flows.push(node);
            // Add to activeFlowConfig
            activeFlowConfig.allNodes[node.id] = jsonClone(node);
            _addNodeToFlowConfig(node);
        } else if (change.op === 'remove') {
            var nodeId = change.nodeId;
            entry.nodeId = nodeId;
            entry.oldConfig = activeFlowConfig.allNodes[nodeId] ? jsonClone(activeFlowConfig.allNodes[nodeId]) : null;
            // Remove from activeConfig.flows array
            activeConfig.flows = activeConfig.flows.filter(function(n) { return n.id !== nodeId; });
            // Remove from activeFlowConfig
            _removeNodeFromFlowConfig(nodeId);
            delete activeFlowConfig.allNodes[nodeId];
        } else if (change.op === 'rewire') {
            var nodeId = change.nodeId;
            entry.nodeId = nodeId;
            entry.oldConfig = activeFlowConfig.allNodes[nodeId] ? jsonClone(activeFlowConfig.allNodes[nodeId]) : null;
            if (activeFlowConfig.allNodes[nodeId]) {
                activeFlowConfig.allNodes[nodeId].wires = jsonClone(change.wires);
                // Update in activeConfig.flows
                for (var j = 0; j < activeConfig.flows.length; j++) {
                    if (activeConfig.flows[j].id === nodeId) {
                        activeConfig.flows[j].wires = jsonClone(change.wires);
                        break;
                    }
                }
                // Update in flow-specific container
                _updateNodeInFlowConfig(nodeId, activeFlowConfig.allNodes[nodeId]);
            }
            entry.newConfig = activeFlowConfig.allNodes[nodeId] ? jsonClone(activeFlowConfig.allNodes[nodeId]) : null;
        } else {
            // op === 'update'
            var nodeId = change.nodeId || (change.node && change.node.id);
            entry.nodeId = nodeId;
            entry.oldConfig = activeFlowConfig.allNodes[nodeId] ? jsonClone(activeFlowConfig.allNodes[nodeId]) : null;
            if (activeFlowConfig.allNodes[nodeId] && change.node) {
                // Merge properties
                var updated = activeFlowConfig.allNodes[nodeId];
                var props = Object.keys(change.node);
                for (var p = 0; p < props.length; p++) {
                    if (props[p] !== 'id') {
                        updated[props[p]] = jsonClone(change.node[props[p]]);
                    }
                }
                // Update in activeConfig.flows
                for (var j = 0; j < activeConfig.flows.length; j++) {
                    if (activeConfig.flows[j].id === nodeId) {
                        for (var p = 0; p < props.length; p++) {
                            if (props[p] !== 'id') {
                                activeConfig.flows[j][props[p]] = jsonClone(change.node[props[p]]);
                            }
                        }
                        break;
                    }
                }
                _updateNodeInFlowConfig(nodeId, updated);
            }
            entry.newConfig = activeFlowConfig.allNodes[nodeId] ? jsonClone(activeFlowConfig.allNodes[nodeId]) : null;
        }
        results.push(entry);
...
function _addNodeToFlowConfig(node) {
    var z = node.z;
    if (node.type === 'tab') {
        activeFlowConfig.flows[node.id] = node;
        activeFlowConfig.flows[node.id].subflows = {};
        activeFlowConfig.flows[node.id].configs = {};
        activeFlowConfig.flows[node.id].nodes = {};
        activeFlowConfig.flows[node.id].groups = {};
    } else if (node.type === 'subflow') {
        activeFlowConfig.subflows[node.id] = node;
        activeFlowConfig.subflows[node.id].configs = {};
        activeFlowConfig.subflows[node.id].nodes = {};
        activeFlowConfig.subflows[node.id].groups = {};
        activeFlowConfig.subflows[node.id].instances = [];
    } else if (node.type === 'group') {
        var container = activeFlowConfig.flows[z] || activeFlowConfig.subflows[z];
        if (container) {
            container.groups[node.id] = node;
        }
    } else {
        var container = activeFlowConfig.flows[z] || activeFlowConfig.subflows[z];
        if (container) {
            ...
            // Global config node
            activeFlowConfig.configs[node.id] = node;
            activeFlowConfig.configs[node.id]._users = [];
        }
    }
}

function _removeNodeFromFlowConfig(nodeId) {
    var node = activeFlowConfig.allNodes[nodeId];
    if (!node) return;
    if (node.type === 'tab') {
        delete activeFlowConfig.flows[nodeId];
    } else if (node.type === 'subflow') {
        delete activeFlowConfig.subflows[nodeId];
    } else if (node.type === 'group') {
        var container = activeFlowConfig.flows[node.z] || activeFlowConfig.subflows[node.z];
        if (container && container.groups) {
            delete container.groups[nodeId];
        }
    } else {
        var container = activeFlowConfig.flows[node.z] || activeFlowConfig.subflows[node.z];
        if (container) {
            delete container.nodes[nodeId];
            delete container.configs[nodeId];
        }
        if (activeFlowConfig.configs[nodeId]) {
            delete activeFlowConfig.configs[nodeId];
        }
    }
}

function _updateNodeInFlowConfig(nodeId, updatedNode) {
    var z = updatedNode.z;
    if (updatedNode.type === 'tab' && activeFlowConfig.flows[nodeId]) {
        // Preserve structural props
        var flow = activeFlowConfig.flows[nodeId];
        Object.keys(updatedNode).forEach(function(k) {
            if (k !== 'subflows' && k !== 'configs' && k !== 'nodes' && k !== 'groups') {
                flow[k] = updatedNode[k];
            }
        });
    } else if (updatedNode.type === 'subflow' && activeFlowConfig.subflows[nodeId]) {
        var sf = activeFlowConfig.subflows[nodeId];
        Object.keys(updatedNode).forEach(function(k) {
            if (k !== 'configs' && k !== 'nodes' && k !== 'groups' && k !== 'instances') {
                sf[k] = updatedNode[k];
            }
        });
        ...
        var container = activeFlowConfig.flows[z] || activeFlowConfig.subflows[z];
        if (container && container.groups) {
            container.groups[nodeId] = updatedNode;
        }
    } else {
        var container = activeFlowConfig.flows[z] || activeFlowConfig.subflows[z];
        if (container) {
            if (container.nodes[nodeId]) {
                container.nodes[nodeId] = updatedNode;
            }
            if (container.configs[nodeId]) {
                container.configs[nodeId] = updatedNode;
            }
        }
        if (activeFlowConfig.configs[nodeId]) {
            activeFlowConfig.configs[nodeId] = updatedNode;
        }
    }
}

function emitNodeError(nodeId, err) {
    events.emit("node-status", {
        id: nodeId,
        status: { fill: "red", shape: "ring", text: "Error: " + err.message }
    });
}

/**
 * Execute runtime changes: stop/start nodes as required by tier.
 * Returns a results object with per-node outcomes.
 */
async function executeRuntimeChanges(entries) {
    var results = { success: [], failed: [] };
    if (!started) {
        // Flows not running, config changes only
        entries.forEach(function(e) {
            results.success.push({ nodeId: e.nodeId, tier: e.tier, action: 'config-only' });
        });
        return results;
    }
    ...
    for (var i = 0; i < entries.length; i++) {
        ...
        try {
            if (tier <= 0) {
                // Tier 0: visual only, no runtime action
                results.success.push({ nodeId: nodeId, tier: tier, action: 'visual' });
            } else if (tier === 1) {
                // Tier 1: wiring - call updateWires on the live node
                var liveNode = getNode(nodeId);
                if (liveNode && entry.newConfig && entry.newConfig.wires) {
                    liveNode.updateWires(entry.newConfig.wires);
                }
                results.success.push({ nodeId: nodeId, tier: tier, action: 'rewire' });
            } else if (tier === 2) {
                // Tier 2: node property edit - stop + recreate
                await _restartNode(nodeId, entry, results);
            } else if (tier === 3) {
                // Tier 3: add node - create instance
                await _createNewNode(nodeId, entry, results);
            } else if (tier === 4) {
                // Tier 4: remove node - stop and remove
                await _stopAndRemoveNode(nodeId, entry, results);
            } else if (tier === 5) {
                // Tier 5: config node edit - stop config node + users, recreate
                await _restartConfigNode(nodeId, entry, results);
            } else if (tier === 6) {
                // Tier 6: flow tab add/remove
                await _handleFlowTabChange(nodeId, entry, results);
            } else if (tier === 7) {
                // Tier 7: subflow template edit - rebuild instances
                await _handleSubflowTemplateChange(nodeId, entry, results);
            }
        } catch (err) {
            emitNodeError(nodeId, err);
            // Rollback config for this node
            if (entry.oldConfig) {
                activeFlowConfig.allNodes[nodeId] = jsonClone(entry.oldConfig);
                _updateNodeInFlowConfig(nodeId, entry.oldConfig);
            }
            results.failed.push({ nodeId: nodeId, tier: tier, error: err.message });
        }
    }
    return results;
}

async function _restartNode(nodeId, entry, results) {
    var flowId = activeNodesToFlow[nodeId];
    var flow = flowId ? activeFlows[flowId] : null;
    if (flow && flow.activeNodes[nodeId]) {
        await flow.stop([nodeId], []);
        delete activeNodesToFlow[nodeId];
    }
    // Recreate
    if (flow && entry.newConfig && entry.newConfig.d !== true) {
        var newNode = await flowUtil.createNode(flow, entry.newConfig);
        if (newNode) {
            flow.activeNodes[nodeId] = newNode;
            activeNodesToFlow[nodeId] = flow.id;
        }
    }
    results.success.push({ nodeId: nodeId, tier: entry.tier, action: 'restart' });
}

async function _createNewNode(nodeId, entry, results) {
    var node = entry.newConfig;
    if (!node || node.d === true) {
        results.success.push({ nodeId: nodeId, tier: entry.tier, action: 'skip-disabled' });
        return;
    }
    var flowId = node.z;
    var flow = activeFlows[flowId] || activeFlows['global'];
    if (flow) {
        var newNode = await flowUtil.createNode(flow, node);
        if (newNode) {
            flow.activeNodes[nodeId] = newNode;
            activeNodesToFlow[nodeId] = flow.id;
        }
    }
    results.success.push({ nodeId: nodeId, tier: entry.tier, action: 'create' });
}

async function _stopAndRemoveNode(nodeId, entry, results) {
    var flowId = activeNodesToFlow[nodeId];
    var flow = flowId ? activeFlows[flowId] : null;
    if (flow && flow.activeNodes[nodeId]) {
        await flow.stop([nodeId], [nodeId]);
        delete activeNodesToFlow[nodeId];
    }
    results.success.push({ nodeId: nodeId, tier: entry.tier, action: 'remove' });
}

async function _restartConfigNode(nodeId, entry, results) {
    // Find which flow owns this config node
    var flowId = activeNodesToFlow[nodeId];
    var flow = flowId ? activeFlows[flowId] : activeFlows['global'];

    // Collect users of this config node
    var usersToRestart = [];
    if (activeFlowConfig.configs[nodeId] && activeFlowConfig.configs[nodeId]._users) {
        usersToRestart = activeFlowConfig.configs[nodeId]._users.slice();
    }

    // Stop config node + all users
    var stopList = [nodeId].concat(usersToRestart);
    if (flow) {
        await flow.stop(stopList, []);
        stopList.forEach(function(id) { delete activeNodesToFlow[id]; });
    }

    // Recreate config node
    if (flow && entry.newConfig && entry.newConfig.d !== true) {
        var newConfigNode = await flowUtil.createNode(flow, entry.newConfig);
        if (newConfigNode) {
            flow.activeNodes[nodeId] = newConfigNode;
            activeNodesToFlow[nodeId] = flow.id;
        }
    }

    // Recreate user nodes
    for (var u = 0; u < usersToRestart.length; u++) {
        var userId = usersToRestart[u];
        var userConfig = activeFlowConfig.allNodes[userId];
        var userFlowId = userConfig ? userConfig.z : null;
        var userFlow = userFlowId ? activeFlows[userFlowId] : activeFlows['global'];
        if (userFlow && userConfig && userConfig.d !== true) {
            var newUserNode = await flowUtil.createNode(userFlow, userConfig);
            if (newUserNode) {
                userFlow.activeNodes[userId] = newUserNode;
                activeNodesToFlow[userId] = userFlow.id;
            }
        }
    }
    results.success.push({ nodeId: nodeId, tier: entry.tier, action: 'restart-config' });
}

async function _handleFlowTabChange(nodeId, entry, results) {
    if (entry.change.op === 'add') {
        // Create new Flow object
        var flowDef = activeFlowConfig.flows[nodeId];
        if (flowDef && !flowDef.disabled) {
            activeFlows[nodeId] = Flow.create(activeFlows['global'], activeFlowConfig, flowDef);
            await activeFlows[nodeId].start();
            var activeNodes = activeFlows[nodeId].getActiveNodes();
            Object.keys(activeNodes).forEach(function(nid) {
                activeNodesToFlow[nid] = nodeId;
            });
        }
        results.success.push({ nodeId: nodeId, tier: entry.tier, action: 'flow-add' });
    } else if (entry.change.op === 'remove') {
        // Destroy Flow object
        if (activeFlows[nodeId]) {
            await activeFlows[nodeId].stop();
            var activeNodes = activeFlows[nodeId].getActiveNodes();
            Object.keys(activeNodes).forEach(function(nid) {
                delete activeNodesToFlow[nid];
            });
            delete activeFlows[nodeId];
        }
        results.success.push({ nodeId: nodeId, tier: entry.tier, action: 'flow-remove' });
    } else {
        // Flow tab property update (label, env, disabled)
        if (activeFlows[nodeId]) {
            activeFlows[nodeId].update(activeFlowConfig, activeFlowConfig.flows[nodeId]);
        }
        results.success.push({ nodeId: nodeId, tier: entry.tier, action: 'flow-update' });
    }
}

async function _handleSubflowTemplateChange(nodeId, entry, results) {
    // Find all instances of this subflow and restart them
    var instanceType = 'subflow:' + nodeId;
    var instancesToRestart = [];
    for (var id in activeFlowConfig.allNodes) {
        if (activeFlowConfig.allNodes[id].type === instanceType) {
            instancesToRestart.push(id);
        }
    }
    for (var k = 0; k < instancesToRestart.length; k++) {
        var instId = instancesToRestart[k];
        var instFlowId = activeNodesToFlow[instId];
        var instFlow = instFlowId ? activeFlows[instFlowId] : null;
        if (instFlow && instFlow.activeNodes[instId]) {
            await instFlow.stop([instId], []);
            delete activeNodesToFlow[instId];
            // Recreate
            var instConfig = activeFlowConfig.allNodes[instId];
            if (instConfig && instConfig.d !== true) {
                var newInst = await flowUtil.createNode(instFlow, instConfig);
                if (newInst) {
                    instFlow.activeNodes[instId] = newInst;
                    activeNodesToFlow[instId] = instFlow.id;
                }
            }
        }
    }
    results.success.push({ nodeId: nodeId, tier: entry.tier, action: 'subflow-rebuild' });
}

/**
 * Apply incremental changes to the running flow configuration.
 * Each change is a ChangeOperation: {id, op, node, nodeId, wires}
 *
 * @param {Array} changes - array of ChangeOperation objects
 * @param {Object} user - the user making the changes
 * @returns {Promise<Object>} - {rev, results: {success:[], failed:[]}}
 */
async function patchFlows(changes, user) {
    if (!activeFlowConfig || !activeConfig) {
        throw new Error("Flows not loaded");
    }
    if (!Array.isArray(changes) || changes.length === 0) {
        return { rev: activeConfig.rev, results: { success: [], failed: [] } };
    }

    // 1. Apply config changes (mutate activeFlowConfig + activeConfig)
    var entries = applyConfigChanges(changes);

    // 2. Execute runtime changes (stop/start nodes)
    var results = await executeRuntimeChanges(entries);

    // 3. Schedule debounced save
    if (settings && settings.flowSaveDebounceMs !== undefined) {
        _flowSaveDebounceMs = settings.flowSaveDebounceMs;
    }
    scheduleDebouncedSave(user);

    // 4. Emit event for comms/broadcast
    events.emit("runtime-event", {
        id: "runtime-deploy",
        payload: { revision: activeConfig.rev, patch: true },
        retain: true
    });

    return { rev: activeConfig.rev, results: results };
}
...
module.exports = {
     ...
     * @param {Array} changes - array of ChangeOperation objects
     * @param {Object} user - the user making the changes
     * @returns {Promise<Object>} - {rev, results}
     */
    patchFlows: patchFlows,

    /**
     * Force an immediate save of any pending debounced changes.
     * Called on shutdown.
