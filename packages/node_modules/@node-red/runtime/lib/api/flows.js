...
var api = module.exports = {
    ...
    */
    ...
    /**
    * Applies incremental changes to the running flow configuration.
    * @param {Object} opts
    * @param {User} opts.user - the user calling the api
    * @param {Array} opts.changes - array of ChangeOperation objects
    * @param {Object} opts.req - the request to log (optional)
    * @return {Promise<Object>} - {rev, results}
    * @memberof @node-red/runtime_flows
    */
    patchFlows: async function(opts) {
        return mutex.runExclusive(async function() {
            runtime.log.audit({event: "flows.patch"}, opts.req);
            try {
                var result = await runtime.flows.patchFlows(opts.changes, opts.user);
                return result;
            } catch(err) {
                runtime.log.warn(runtime.log._("api.flows.error-save",{message:err.message}));
                runtime.log.warn(err.stack);
                throw err;
            }
        });
